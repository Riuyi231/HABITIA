...
const METODO_ETIQ = { efectivo: 'Efectivo', transferencia: 'Transferencia', tarjeta: 'Tarjeta', cheque: 'Cheque' };
...
async function verMorosidad() {
  ...
  modal('Morosidad · 30/60/90', `
    ...
    ${pg.total
      ...
      <button class="btn secundario" onclick="cerrarModal()">Cerrar</button>`, 820);
}
...
async function viewCobros() {
  ...
  $('#content').innerHTML = `
    ...
    <div class="card" style="margin-bottom:16px">
      <div class="detalle">
        <b>Cobrado:</b> <span style="color:var(--verde)">${RD$(cobrado)}</span> ·
        <b>Pendiente:</b> <span style="color:${pendiente ? 'var(--rosado)' : 'var(--verde)'}">${RD$(pendiente)}</span>
        ${vencido > 0 ? ` · <b>Vencido:</b> <span style="color:var(--rosado)">${RD$(vencido)}</span>` : ''}
        <span class="detalle" style="margin-left:12px;color:var(--texto-suave)">Al registrar un pago se genera el recibo numerado automáticamente.</span>
      </div>
    </div>
    <div class="card">
      <div class="tabla-scroll">
      <table>
        ...
        <tbody>
          ${pg.filas.map((r) => `<tr>
            ...
            <td class="num">${Number(r.abonado) > 0 ? RD$(r.abonado) : '—'}</td>
            <td class="num"><b ${Number(r.saldo) > 0 ? 'style="color:var(--rosado)"' : ''}>${Number(r.saldo) > 0 ? RD$(r.saldo) : '—'}</b></td>
...
function pagarCobro(id) {
  (async () => {
    ...
    modal('Cobrar alquiler · ' + (fila.estudio_nombre || 'estudio'), `
      <div class="detalle" style="margin-bottom:10px">
        Inquilino: <b>${esc(fila.inquilino_nombre || '—')}</b> · Alquiler: <b>${RD$(fila.monto)}</b>
        ${Number(fila.abonado) > 0 ? ` · Ya abonado: <b>${RD$(fila.abonado)}</b> · Falta: <b>${RD$(fila.saldo)}</b>` : ''}
      </div>
      <form onsubmit="guardarPago(event, ${id})">
        <div class="form-grid">
          <div class="campo"><label>Fecha de pago</label><input id="p-fecha" type="date" value="${hoyISO()}"></div>
          <div class="campo"><label>Monto a cobrar (RD$)</label><input id="p-monto" type="number" min="0.01" step="0.01" value="${fila.saldo}" required></div>
        ...
        <div class="detalle" style="margin:6px 0 12px;color:var(--texto-suave)">Al confirmar se generará el recibo <b>numerado</b> y quedará guardado en el historial.</div>
...
function abonarCobro(id) {
  (async () => {
    ...
    modal('Registrar abono', `
      <div class="detalle" style="margin-bottom:10px">Abonado hasta ahora: <b>${RD$(abonado)}</b></div>
      <form onsubmit="guardarAbono(event, ${id})">
        <div class="form-grid">
          <div class="campo"><label>Monto (RD$) *</label><input id="a-monto" type="number" min="1" step="0.01" required autofocus></div>
...
async function viewGastos() {
  ...
  const gastos = (await window.api.gastosList({ mes: mesActual, categoria: gF.categoria || '', subcategoria: gF.subcategoria || '', proveedor: gF.proveedor || '' })).data;
  ...
  $('#content').innerHTML = `
    ...
    <div class="card">
      <div class="tabla-scroll">
      <table>
        <thead><tr><th>Fecha</th><th>Tipo</th><th>Estudio</th><th>Proveedor</th><th>Concepto</th><th>Método</th><th class="num">Monto</th><th></th></tr></thead>
        <tbody>
          ${pg.filas.map((g) => `<tr>
            <td>${esc(g.fecha)}</td>
            ...
            <td class="num"><b>${RD$(g.monto)}</b></td>
            <td style="text-align:right">
      ...
      ${gastos.length ? `<div style="text-align:right;padding:10px 12px;font-weight:700">Total: ${RD$(gastos.reduce((s, g) => s + Number(g.monto), 0))}</div>` : `<div class="vacio">Sin gastos para los filtros elegidos.</div>`}
    </div>`;
}
    ...
    modal('Nuevo gasto', `
      <form onsubmit="guardarGasto(event)">
        <div class="form-grid">
          ...
          <div class="campo"><label>Monto (RD$) *</label><input id="g-monto" type="number" min="0" step="0.01" required autofocus></div>
...
async function viewMantenimiento() {
  ...
  const costoAcumulado = ordenes.filter((o) => o.estado === 'cerrada').reduce((s, o) => s + Number(o.costo || 0), 0);
  const filtro = (window.__mantFiltro || 'todos');
  const ver = (o) => (filtro === 'abiertas' ? o.estado === 'abierta' : filtro === 'cerradas' ? o.estado === 'cerrada' : true);
  ...
  $('#content').innerHTML = `
    <div class="btn-row">
      ...
      <select id="mant-filtro" onchange="window.__mantFiltro=this.value;__pag.mantenimiento=1;viewMantenimiento()" style="border:1px solid var(--borde);border-radius:8px;padding:8px 10px;background:var(--fondo)">
        ...
        <option value="abiertas" ${filtro === 'abiertas' ? 'selected' : ''}>Abiertas</option>
        <option value="cerradas" ${filtro === 'cerradas' ? 'selected' : ''}>Cerradas</option>
      </select>
      <span style="flex:1"></span>
      <span class="detalle" style="align-self:center">${abiertas.length} abiertas · ${ordenes.length - abiertas.length} cerradas · costo total ${RD$(costoAcumulado, 0)}</span>
    </div>
    <div class="card">
      ${visibles.length
        ? `<div class="tabla-scroll"><table><thead><tr><th>#</th><th>Estudio</th><th>Inquilino</th><th>Detalle</th><th>Prioridad</th><th>Proveedor</th><th class="num">Est. costo</th><th class="num">Costo real</th><th>Estado</th><th></th></tr></thead>
           <tbody>${pg.filas.map((o) => `<tr>
             ...
             <td class="num">${Number(o.costo_estimado) > 0 ? RD$(o.costo_estimado) : '—'}</td>
             <td class="num"><b ${Number(o.costo) > 0 ? 'style="color:var(--rosado)"' : ''}>${Number(o.costo) > 0 ? RD$(o.costo) : '—'}</b></td>
             <td><span class="badge ${o.estado === 'abierta' ? 'vencida' : 'pagada'}">${o.estado === 'abierta' ? 'Abierta' : 'Cerrada'}</span></td>
             <td style="text-align:right">
               ${o.estado === 'abierta' ? `<button class="btn pequeño secundario" onclick="cerrarOrden(${o.id})">Cerrar</button>` : ''}
...
async function guardarOrden(e) {
  ...
  if (!r.ok) return toast(r.error, 'error');
  cerrarModal(); toast('Orden #' + r.data.id + ' abierta'); vista('mantenimiento');
}
